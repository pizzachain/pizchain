...
#error PIZ VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, PIZVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, PIZVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic PIZVMOC_INTRINSIC_INIT_PRIORITY("eosvmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
...
static void eosio_exit(int32_t code) {
   siglongjmp(*eos_vm_oc_get_jmp_buf(), PIZVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
...
static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(eos_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*eos_vm_oc_get_jmp_buf(), PIZVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_PIZVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function PIZVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(#module "." #name))).value() \
   ); \
	void name()

DEFINE_PIZVMOC_TRAP_INTRINSIC(eosvmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_PIZVMOC_TRAP_INTRINSIC(eosvmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_PIZVMOC_TRAP_INTRINSIC(eosvmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_PIZVMOC_TRAP_INTRINSIC(eosvmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_PIZVMOC_TRAP_INTRINSIC(eosvmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. PIZ VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      ...
      case PIZVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case PIZVMOC_EXIT_SEGV:
         PIZ_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case PIZVMOC_EXIT_EXCEPTION: //exception
