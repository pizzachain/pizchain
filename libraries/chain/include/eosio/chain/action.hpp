...
namespace eosio { namespace chain {
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         PIZ_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         PIZ_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
