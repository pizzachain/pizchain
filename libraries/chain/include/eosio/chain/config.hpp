...
const static name system_account_name    { N(piz) };
const static name null_account_name      { N(piz.null) };
const static name producers_account_name { N(piz.prods) };
...
const static name eosio_auth_scope       { N(piz.auth) };
const static name eosio_all_scope        { N(piz.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name eosio_any_name  { N(piz.any) };
const static name eosio_code_name { N(piz.code) };
...
constexpr uint64_t PIZ_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / eosio::chain::config::percent_100;
}

template<typename Number>
Number PIZ_PERCENT_CEIL(Number value, uint32_t percentage) {
