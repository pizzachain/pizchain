...
class memory {
      ...
      static_assert(sizeof(control_block) <= wcb_allowance, "PIZ VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(-cb_offset == PIZ_VM_OC_CONTROL_BLOCK_OFFSET, "PIZ VM OC control block offset has slid out of place somehow");
      static_assert(stride == PIZ_VM_OC_MEMORY_STRIDE, "PIZ VM OC memory stride has slid out of place somehow");
