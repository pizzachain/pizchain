...
struct intrinsic_invoker_impl<is_injected, Ret, std::tuple<T &, Inputs...>, std::tuple<Translated...>> {
   ...
   template<then_type Then, typename U=T>
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<std::is_const<U>::value, Ret> {
      PIZ_ASSERT((U32)ptr != 0, wasm_exception, "references cannot be created for null pointers");
   ...
   template<then_type Then, typename U=T>
   static auto translate_one(Inputs... rest, Translated... translated, I32 ptr) -> std::enable_if_t<!std::is_const<U>::value, Ret> {
      PIZ_ASSERT((U32)ptr != 0, wasm_exception, "reference cannot be created for null pointers");
      T &base = *array_ptr_impl<T>((uint32_t)ptr, 1);
...
#define _REGISTER_PIZVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   static eosio::chain::eosvmoc::intrinsic _INTRINSIC_NAME(__intrinsic_fn, __COUNTER__) PIZVMOC_INTRINSIC_INIT_PRIORITY (\
      MOD "." NAME,\
