...
#else
#define _REGISTER_PIZVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
   ...
   struct wasm_interface_impl {
      ...
      wasm_interface_impl(wasm_interface::vm_type vm, bool eosvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const eosvmoc::config& eosvmoc_config) : db(d), wasm_runtime_time(vm) {
...
#ifdef PIZ_EOS_VM_RUNTIME_ENABLED
...
#endif
#ifdef PIZ_EOS_VM_JIT_RUNTIME_ENABLED
         ...
         if(!runtime_interface)
            PIZ_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef PIZ_VM_OC_RUNTIME_ENABLED
         if(eosvmoc_tierup) {
            PIZ_ASSERT(vm != wasm_interface::vm_type::eos_vm_oc, wasm_exception, "You can't use PIZ VM OC as the base runtime when tier up is activated");
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            PIZ_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            PIZ_ASSERT(module.memories.defs.size(), wasm_exception, "");
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  PIZ_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  PIZ_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_PIZ_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_PIZVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, PIZ_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
