...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::eosio_code_name ) // virtual piz.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         PIZ_THROW( action_validate_exception,
 ...
 */
void apply_piz_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   PIZ_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   PIZ_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   PIZ_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   PIZ_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_metadata_object, by_name>(create.creator);
   if( !creator.is_privileged() ) {
      PIZ_ASSERT( name_str.find( "eosio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'eosio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   PIZ_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_piz_setcode(apply_context& context) {
   ...
   PIZ_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   PIZ_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   PIZ_ASSERT( code_size > 0 || existing_code, set_exact_code, "contract is already cleared" );
   ...
   if( existing_code ) {
      ...
      PIZ_ASSERT( old_code_entry.code_hash != code_hash, set_exact_code,
...
void apply_piz_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_piz_updateauth(apply_context& context) {
   ...
   PIZ_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   PIZ_ASSERT( update.permission.to_string().find( "eosio." ) != 0, action_validate_exception,
               "Permission names that start with 'eosio.' are reserved" );
   PIZ_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   PIZ_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      PIZ_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      PIZ_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      PIZ_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      PIZ_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      PIZ_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_piz_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   PIZ_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   PIZ_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
...
void apply_piz_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );

   auto requirement = context.get_action().data_as<linkauth>();
   try {
      PIZ_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      PIZ_ASSERT(account != nullptr, account_query_exception,
      ...
      PIZ_ASSERT(code != nullptr, account_query_exception,
      ...
      if( requirement.requirement != config::eosio_any_name ) {
         ...
         PIZ_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         PIZ_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_piz_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   PIZ_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_piz_canceldelay(apply_context& context) {
