...
struct report_time {
    ...
    void report() {
        ...
        ilog("piz-blocklog - ${desc} took ${t} msec", ("desc", _desc)("t", duration));
    }
...
void blocklog::read_log() {
   ...
   PIZ_ASSERT( end, block_log_exception, "No blocks found in block log" );
   PIZ_ASSERT( end->block_num() > 1, block_log_exception, "Only one block found in block log" );
...
void smoke_test(bfs::path block_dir) {
   ...
   PIZ_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file", ("file", td.block_file_name.string())("pos", sizeof(uint64_t)) );
   uint64_t file_pos;
   auto size = fread((void*)&file_pos, sizeof(uint64_t), 1, td.blk_in);
   PIZ_ASSERT( size == 1, block_log_exception, "${file} read fails", ("file", td.block_file_name.string()) );
   status = fseek(td.blk_in, file_pos + trim_data::blknum_offset, SEEK_SET);
   PIZ_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file", ("file", td.block_file_name.string())("pos", file_pos + trim_data::blknum_offset) );
   uint32_t bnum;
   ...
   PIZ_ASSERT( td.last_block == bnum, block_log_exception, "blocks.log says last block is ${lb} which disagrees with blocks.index", ("lb", bnum) );
...
int main(int argc, char** argv) {
   ...
   options_description cli ("piz-blocklog command line options");
   try {
      blocklog blog;
