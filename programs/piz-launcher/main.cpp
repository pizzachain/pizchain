...
struct launcher_def {
   ...
   std::map<uint,string> specific_nodepiz_args;
   std::map<uint,string> specific_nodepiz_installation_paths;
   testnet_def network;
   string gelf_endpoint;
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodepiz_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodepiz_installation_paths);
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodepiz_args[num] = specific_args[i];
    }
  }

  char* erd_env_var = getenv ("PIZ_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable PIZ_HOME is set to the build path." << endl;
    exit (-1);
  }
...
launcher_def::launch (eosd_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodepiz_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodepiz_installation_paths.count(node_num)) {
        install_path = specific_nodepiz_installation_paths[node_num] + "/";
     }
  }
  string eosdcmd = install_path + "programs/nodepiz/" + string(node_executable_name) + " ";
  ...
  if (instance.name != "bios" && !specific_nodepiz_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodepiz_args.count(node_num)) {
        eosdcmd += specific_nodepiz_args[node_num] + " ";
     }
  }
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/piz-tn_bounce.sh " + eosd_extra_args;
      if (node_num != "bios" && !specific_nodepiz_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodepiz_args.count(node_num_i)) {
            cmd += " " + specific_nodepiz_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "PIZ_HOME", host.eosio_home }, { "PIZ_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/piz-tn_down.sh ";
      do_command(host, node.name,
                 { { "PIZ_HOME", host.eosio_home }, { "PIZ_NODE", node_num }, { "PIZ_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/piz-tn_roll.sh ";
      do_command(*host, host_name, { { "PIZ_HOME", host->eosio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the piz-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the piz-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the piz-tn_roll.sh script")
