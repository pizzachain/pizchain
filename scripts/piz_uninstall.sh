...
export PIZ_INSTALL_DIR=${INSTALL_LOCATION:-$PIZ_INSTALL_DIR}

if [[ ! -d "${PIZ_INSTALL_DIR}" ]]; then
   echo "[PIZ installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${PIZ_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/eosio/eos/issues/6940
   echo "[PIZ installation found: ${PIZ_INSTALL_DIR}]" && INSTALL_PATHS+=("${PIZ_INSTALL_DIR}") # PIZ_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${PIZ_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            echo " - PIZ Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled PIZ Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing PIZ Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/piz_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
