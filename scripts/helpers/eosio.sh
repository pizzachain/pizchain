...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- PIZ is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile PIZ${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile PIZ${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "PIZ_INSTALL_DIR: ${PIZ_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}PIZ compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where PIZ is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${PIZ_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " PIZ_INSTALL_DIR;
                export PIZ_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
        ...
        export PIZ_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "PIZ will be installed to: ${PIZ_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $PIZ_INSTALL_DIR ]]; then
    echo "PIZ has already been installed into ${PIZ_INSTALL_DIR}... It's suggested that you piz_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}PIZ website:${COLOR_NC} https://www.pizex.io"
    echo "${COLOR_CYAN}PIZ Telegram channel:${COLOR_NC} "
    echo "${COLOR_CYAN}PIZ resources:${COLOR_NC} "
    echo "${COLOR_CYAN}PIZ Stack Exchange:${COLOR_NC} "
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the PIZ build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${PIZ_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
